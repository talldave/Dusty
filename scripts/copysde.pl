...
my %opt = &ReadCommandLine('pre:post:s:t:o:file:M:tl:k:e2007');
&requiredOptions('s:t:o');
&setOption("p","$opt{t}");
if ("$opt{s}" =~ /agol/i) { system "/mangomap1/users/sde//.agol2"; }
&BeginProgram;
...
my $shape = "shape"; 
if (defined $opt{k}) { $key = "$opt{k}"; } else { $key = "$t_user"."_defaults"; }
...
#
	#my $sdeexport2 = "sdeexport -o create -l $f_table,$shape -f - -s $f_serv -i $f_inst -u $f_user -p $f_pass ";
...
#	
	#&normal_io("$t_table");
&export_import("$opt{s}","$opt{t}","$opt{o}","$f_table","$t_table");
	...
	#my $srcCount = &getCount("$f_table","$opt{s}");
